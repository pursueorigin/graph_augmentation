"""
Main script to train a GIN classifier on molecular graphs with augmentation.
"""
import argparse
import json
import math
...
def parse_args():
    ...
    parser.add_argument('--aug-size', type=float, default=0.5)  # SubMix
...
def eval_acc(model, loader, device):
    model.eval()
    count_sum, acc_sum = 0, 0
    for data in loader:
        data = data.to(device)
        output = model(data.x, data.edge_index, data.batch)
        acc = (output.argmax(dim=1) == data.y).double().mean()
        acc_sum += acc.item() * len(data.y)
